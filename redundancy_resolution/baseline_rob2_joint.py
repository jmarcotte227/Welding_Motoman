...
def main():
    ...
    positioner=positioner_obj(
        'D500B',
        def_path='../config/D500B_robot_default_config.yml',
    ...
    with open(data_dir+'sliced_meta.yml', 'r') as file:
        ...
        for i in range(slicing_meta['layer_num']):
            ...
            for x in range(num_sections):
                curve_sliced_js_ith_layer.append(np.loadtxt(
        ...
        for i in range(slicing_meta['baselayer_num']):
            ...
            for x in range(num_sections):
                curve_sliced_js_base_ith_layer.append(np.loadtxt(
                    data_dir+'curve_sliced_js/MA2010_base_js'+str(i)+'_'+str(x)+'.csv',
                    delimiter=','
                ).reshape((-1,6)))
        ...
        distance=500
        rob2_curve_js_base=rr.rob2_flir_resolution(
            curve_sliced_js_base,
            robot2,
        ...
        for i in range(slicing_meta['baselayer_num']):
            for x in range(len(rob2_curve_js_base[i])):
                np.savetxt(
                    data_dir+'curve_sliced_js/MA1440_base_js'+str(i)+'_'+str(x)+'.csv',
                    rob2_curve_js_base[i][x],
                    delimiter=','
                )
