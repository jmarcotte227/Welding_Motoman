...
def lstm_linearization_inc(model, h_0, c_0, x_0):
    '''
    To be used for incremental linearization. Only returns the B matrix between 
    control input and height output. Returns C as well
    '''
    # takes the model, hidden state opperating points and input operating
    # point and produces the linearized system matrices. 
    h_dim = h_0.shape[0]
    ...
    f = sigmoid(W_hf.T@h_0+W_uf.T@x_0+b_f)
    i = sigmoid(W_hi.T@h_0+W_ui.T@x_0+b_i)
    tc = tanh(W_hc.T@h_0+W_uc.T@x_0+b_c)
    o = sigmoid(W_ho.T@h_0+W_uo.T@x_0+b_o)
    c = f*c_0+i*tc

    ##### Compute A_hh #####
    # df_dh = W_hf@sigmoid_p(W_hf.T@h_0+W_uf.T@x_0+b_f)
    # di_dh = W_hi@sigmoid_p(W_hi.T@h_0+W_ui.T@x_0+b_i)
    # dtc_dh = W_hc@tanh_p(W_hc.T@h_0+W_uc.T@x_0+b_c)
    ...
    # ##### Compute B_h #####
    # df_du = W_uf@sigmoid_p(W_hf.T@h_0+W_uf.T@x_0+b_f)
    # di_du = W_ui@sigmoid_p(W_hi.T@h_0+W_ui.T@x_0+b_i)
    # dtc_du = W_uc@tanh_p(W_hc.T@h_0+W_uc.T@x_0+b_c)

    # dc_du = df_du@diag(c_0)+di_du@diag(tc)+dtc_du@diag(i)
    # do_du = W_uo@sigmoid_p(W_ho.T@h_0+W_uo.T@x_0+b_o)
    ...
    ##### Compute B_h one line #####
    dh_du = (W_uo@sigmoid_p(W_ho.T@h_0+W_uo.T@x_0+b_o))@diag(tanh(c))+(((W_uf@sigmoid_p(W_hf.T@h_0+W_uf.T@x_0+b_f))@diag(c_0)+(W_ui@sigmoid_p(W_hi.T@h_0+W_ui.T@x_0+b_i))@diag(tc)+(W_uc@tanh_p(W_hc.T@h_0+W_uc.T@x_0+b_c))@diag(i))@tanh_p(c))@diag(o)
    B_h = dh_du.T
