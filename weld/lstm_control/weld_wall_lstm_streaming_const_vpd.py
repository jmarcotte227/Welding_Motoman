...
def main():
    ...
    for layer in range(num_layer_start, num_layer_end):
        ...
        if RECORDING:
            ...
            SS.start_recording()
        if ARCON:
            fronius_client.job_number = int(feed_cmd/10+JOB_OFFSET)
            fronius_client.start_weld()
        if ONLINE:
            SS.init_motion()
        while lam_cur<lam_relative[-1] - v_cmd/STREAMING_RATE:
            ...
            if seg_idx != v_cor_idx:
                ...
                feed_cmd = VPD*v_cmd
                if ARCON:
                    fronius_client.async_set_job_number(int(round(feed_cmd/10)+JOB_OFFSET), welder_handler)

                lstm_pred.append(torch.squeeze(reg.unreg(y_out_cont.detach(),1)))

            v_cmds.append(v_cmd)
            feed_cmds.append(feed_cmd)
        ...
        if RECORDING:
            ...
            np.savetxt(save_path + "feed_cmd.csv", np.array(feed_cmds), delimiter=",")
