...
with open(data_dir+'bead_params.pkl', 'rb') as file:
			bead_params = pickle.load(file)
...
for layer in range(1):#range(num_layer_start,num_layer_end,nominal_slice_increment):
	vel_idx = 0
	...
	for x in range(0,num_sections): ######change back to 0
		...
		for j in range(0,len(breakpoints)):
			...
			v1_all.append(max(bead_params[bead_id[x]][1],0.1))
		...
		print('V1: ', v1_all)
		print("feedrate: ,", bead_params[bead_id[x]][0])
		print("layer: ", layer)
