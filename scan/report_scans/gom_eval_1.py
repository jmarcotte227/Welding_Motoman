...
def pcd2height(scanned_points,z_height_start,bbox_min=(-40,-20,0),bbox_max=(40,20,45),\
                resolution_z=0.1,windows_z=0.2,resolution_x=0.1,windows_x=1,stop_thres=20,\
                stop_thres_w=10,use_points_num=10,width_thres=0.8,Transz0_H=None):
    ...
    bbox_mesh = o3d.geometry.TriangleMesh.create_box(width=bbox_max[0]-bbox_min[0], height=bbox_max[1]-bbox_min[1], depth=30)
    box_move=np.eye(4)
    ...
    for z in np.arange(z_height_start,z_max+resolution_z,resolution_z):
        ...
        for x in np.arange(bbox_min[0],bbox_max[0]+resolution_x,resolution_x):
            ...
            z_heights= np.asarray(welds_points_x.points)[:,2]
            profile_p.append(np.array([x,this_width,z_heights],dtype='object'))
        profile_p = np.array(profile_p)
        
        for pf_i in range(len(profile_p)):
            try:
                profile_height[profile_p[pf_i][0]]=np.append(profile_height[profile_p[pf_i][0]],(profile_p[pf_i][2]))
            except KeyError:
                profile_height[profile_p[pf_i][0]] = profile_p[pf_i][2]

    profile_height_arr = []
    for x in profile_height.keys():
        profile_height_arr.append(np.array([x,profile_height[x]],dtype='object'))
    profile_height_arr=np.array(profile_height_arr,dtype='object')
...
if __name__ == "__main__":
    ...
    scanned_points_mesh = o3d.io.read_triangle_mesh(data_dir+'1layer_bent_wall_230Matrix_280Fill.stl')
    ...
    translation_p = np.array([-5,1,0]) ## CHANGE THIS TO CALIBRATE NEW SAMPLE
    ...
    for j in range(1): #row index
        for i in range(1): #col index
            ...
            bbox_max=(x_offset+box_width-row_offset*i,y_offset+box_height-col_offset*j,45)
    ...
    # plt.show()
    print(np.array(all_heights, dtype='object').shape)


    pickle.dump(all_heights, open(data_dir+'gom_data_1.pickle','wb'))
