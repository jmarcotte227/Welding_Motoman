...
def main():
    ...
    num_layers = 106
    points_per_layer=50
    point_distance = line_length/points_per_layer
    vertical_shift = 4 #mm  
    ...
    #rotation criteria
    layer_angle = np.arcsin((max_dH-min_dH)/(np.sqrt(2)/2*line_length))
    rot_point = max_dH/np.tan(layer_angle)-(np.sqrt(2)/2*line_length)/2
    ...
    # generate line with 45 deg angle in the x,y plane
    line_points = [((point_distance*x-(line_length/2))*np.sqrt(2)/2,
                    (point_distance*x-(line_length/2))*np.sqrt(2)/2) for x in range(0,points_per_layer+1)]

    #base layer
    print(len(line_points)-1)
    for i in range(len(line_points)-1):
        base_layer[i,0]=line_points[i][0]
        base_layer[i,1]=line_points[i][1]
        base_layer[i,-1]=-1
    

    #first layer
    for i in range(len(line_points)-1):
        curve_curved[i,0]=line_points[i][0]
        curve_curved[i,1]=line_points[i][1]
        curve_curved[i,-1]=-1
    ...
    for layer in range(num_layers*slices_per_layer):
        np.savetxt(
                'slice_ER_4043/curve_sliced/slice'+str(layer+1)+'_0.csv',
