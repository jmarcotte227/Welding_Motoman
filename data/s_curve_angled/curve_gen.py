...
def main():
    ...
    curve_points_x = np.linspace(0,curve_seg_length, points_per_layer)
    curve_points_y = np.linspace(0,curve_seg_length*2, points_per_layer)

    #base layer
    for i in range(len(curve_points_x)):
        base_layer[i,0]=curve_points_x[i]
        base_layer[i,1]=curve_points_y[i]
        base_layer[i,-1]=-1

   #first layer
    for i in range(len(curve_points_x)):
        curve_curved[i,0]=curve_points_x[i]
        curve_curved[i,1]=curve_points_y[i]
        curve_curved[i,-1]=-1
